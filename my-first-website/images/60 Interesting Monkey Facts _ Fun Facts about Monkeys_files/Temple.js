...
function Temple (config) {
	...
	this.utils = (function() {
		...
		var fitText = function(t) {
			...
			var l = Number(window.getComputedStyle(p,null).getPropertyValue('line-height').replace('px','')) || (s + 1);
...
temple.NetflixBanner = (function(_super) {

	__extends(NetflixBanner, _super);

	function NetflixBanner() {
		_super.call(this, arguments[0]);
	}

	NetflixBanner.prototype.init = function() {
		this.chain(this.setupBanner)
			// .chain(this.setupTimeline)
			// .chain(this.setupListeners)
			// .chain(this.show)	
	}

	NetflixBanner.prototype.setupBanner = function() {
		this.locale 	= Monet.getComponentLocale("text.Tune_In").substr(0,2);
		this.safe 		= this.monetData.rootAssets["bool.Safe"].value;
		this.highTouch 	= this.monetData.rootAssets["bool.High_touch"].value;
		...
		//load svg
		var svgObj = {}
		svgObj = {xhr:new XMLHttpRequest(), el:$("#logoSVG")};
		svgObj.xhr.onload = function(e) {
			var r = e.currentTarget.responseXML.documentElement;
			r.setAttribute('class', svgObj.el.getAttribute('class'));
			var id = svgObj.el.getAttribute('id');
			r.setAttribute('id', id);
			svgObj.el.parentNode.replaceChild(r, svgObj.el);
			window[id] = r;
			this.setupTimeline();
		}.bind(this);
		svgObj.xhr.open('GET', this.monetData.rootAssets['image.Title_Treatment'].url, !0);
		svgObj.xhr.overrideMimeType("image/svg+xml");
		svgObj.xhr.send("");


		$("#titleTreatment").classList.add(this.locale);
		$("#mainCopyComponent").classList.add(this.locale);
		$(".ctaContainer").classList.add(this.locale);
		$("#logoComponent").classList.add(this.locale);
		// $(".ratingBug").classList.add(this.locale);
		$("#supercutCopyComponent").classList.add(this.locale);
		...
		this.highTouchElements = ["#mainCopyComponent", "#logoComponent", ".ctaContainer", "#greyPanel"];
		TweenMax.set(this.highTouchElements, { className: "+=highTouch_" + this.highTouch });
	}

	// private methods

	NetflixBanner.prototype.setupTimeline = function() {
		this.mainTimeline = new TimelineMax({ paused: true });

		if (this.highTouch) {
			this.mainTimeline
				.from(".ctaContainer", 1 ,{ opacity: 0 }, "start+=5")
				.from("#mainCopyComponent", 1 ,{ opacity: 0 }, "start+=5")
				.from("#logoComponent", 1 ,{ opacity: 0 }, "start+=5")
				.from('.gradient', 1, {opacity:0, ease: Power1.easeInOut }, "start+=5")
				.call($('#logoComponent').play, null, $('#logoComponent'), "start+=5")
		} else {
			this.mainTimeline
				.staggerFrom(["#mainCopyComponent", ".ctaContainer"], 1 ,{ opacity: 0 }, 0.2, "start+=5")
		}
	...
	NetflixBanner.prototype.setupListeners = function() {
		$('#ribbonComponent').addEventListener("leftPillarComplete", function() {
			if (temple.isMobile || !this.monetData.rootAssets["bool.Supercut_Toggle"].value){
				this.startAnimation();
			} else {
				$("#supercutComponent").play();
				$("#supercutLogoComponent").progress(1);
				TweenMax.to($('.supercutOverlay'), .3, { opacity: 1, ease: Quad.easeOut });
			}
		}.bind(this));
		...
		$('#ribbonComponent').addEventListener("click", this.exit.bind(this));

		$('#supercutComponent').addEventListener("video-complete", this.startAnimation.bind(this));
		$('#supercutComponent').addEventListener("video-close", this.startAnimation.bind(this));
		...
		$('#trailerComponent').addEventListener("video-close", this.closeTrailer.bind(this));
		$('#trailerComponent').addEventListener("video-complete", this.closeTrailer.bind(this));
		$('#trailerComponent').addEventListener("video-click", this.onBannerClick.bind(this));

		$('.trailerHit').addEventListener("mouseover", this.trailerOver.bind(this));
		$('.trailerHit').addEventListener("mouseout", this.trailerOut.bind(this));
		$('.trailerHit').addEventListener("click", this.initTrailer.bind(this));

		this.createdMobileDiv = false;

		this.show();
	}

	NetflixBanner.prototype.startAnimation = function() {
		this.mainTimeline.play();
		$('#ctaComponent').resize();	
		$('.supercutOverlay').hide();
	}

	NetflixBanner.prototype.trailerOver = function() {
		if(!temple.isMobile){
			TweenMax.to($('#trailerPlayButton'),0.5, {backgroundColor: "#fff", ease:Power0.easeNone})
			TweenMax.to($('#polygon'),0.5, {fill: "#d51b28", ease:Power0.easeNone})
		}
	}

	NetflixBanner.prototype.trailerOut = function() {
		if(!temple.isMobile){
			TweenMax.to($('#trailerPlayButton'),0.2, {backgroundColor: "#d51b28", ease:Power0.easeNone})
			TweenMax.to($('#polygon'),0.2, {"fill": "#fff", ease:Power0.easeNone})
		}
	}

    NetflixBanner.prototype.initTrailer = function() {
    	this.mainTimeline.progress(1);

		if (temple.isMobile || temple.isSafari) {
			this.playTrailer();
		} else {
			
		}
		this.trailerTimeline.play(0).timeScale(1);
		// this.trailerTimeline.progress(1);
	}

	NetflixBanner.prototype.playTrailer = function() {

		this.trailerScreen = true;
		$('#trailerComponent').play();

		if(this.isSafari || this.isiOS){
			this.myVideo = $(".netflix-video-container")[0].children[0];
			this.myVideo.removeAttribute('controls');

			if(!this.createdMobileDiv){
				this.createdMobileDiv = true;

				this.mobileClick = document.createElement("img");
				this.mobileClick.classList = "mobileClick";
				$(".netflix-video-container")[0].appendChild(this.mobileClick);
			}

			TweenMax.set($(".controls")[0], {className:"-=hide"});
			$(".mobileClick").addEventListener("click", function(){
				TweenMax.set($(".controls")[0], {autoAlpha:1});
			}.bind(this));
	    }

		// $('#trailerComponent').querySelector('.bar').hide();
		// $('#trailerComponent').querySelector('.timer').hide();
		TweenMax.set($('#videoGradientWrapper'),{visibility:"visible"});
	}

	NetflixBanner.prototype.closeTrailer = function(e) {
		TweenMax.set($('#videoGradientWrapper'),{ visibility:"hidden"});
    	this.mainTimeline.progress(1);
    	// this.trailerTimeline.reverse().timeScale(1);
		this.trailerTimeline.reverse().progress(0);
		TweenMax.set([$('.trailerHit'), $('#trailerPlayButton')], { autoAlpha:1});
	}

	// Event Handlers


	NetflixBanner.prototype.onShow = function() {
		$('#ribbonComponent').play();
	}

	NetflixBanner.prototype.onSupercutTime = function(e) {
	...
	NetflixBanner.prototype.onOver = function() {
		if (!temple.isMobile) {
			$('#ctaComponent').mouseover();
		}
	}

	NetflixBanner.prototype.onOut = function() {
		if (!temple.isMobile) {
			$('#ctaComponent').mouseout();	
		}
	}

    NetflixBanner.prototype.onExit = function(e) {

	 	$('#trailerComponent').close();
	 	$('#supercutComponent').close();
		$('#ribbonComponent').progress(1, true);
		$("#logoComponent").progress(1, true);
		$('#ribbonComponent').hide();
		$('.ribbonClick ').hide();

		this.mainTimeline.progress(1);
		this.trailerTimeline.progress(0);
		TweenMax.set($('#videoGradientWrapper'),{ visibility:"hidden"});	

		setTimeout(function() { $('#ctaComponent').mouseout() }, 1000);
	}

	return NetflixBanner;
...
temple.Banner = ( function(_super) { 
	...
	banner.prototype.init = function() {
		...
		this.blinkRolloverAllowed = true;
		this.trailerScreen = false;

		// $(".bannerClick")[0].addEventListener("mousemove", this.onMouseMove.bind(this));
	}

	banner.prototype.setupTimeline = function() {
		_super.prototype.setupTimeline.call(this); //call parent function first before handling code below

		this.svgLeft = $('#logoSVG').getElementById("svgLeft");
		this.svgMiddle = $('#logoSVG').getElementById("svgMiddle");
		this.svgRight = $('#logoSVG').getElementById("svgRight");
		this.svgBottom = $('#logoSVG').getElementById("svgBottom");
		
		this.mainTimeline.from($('.keyartWrapper'), 1.5, { opacity:0 ,ease: Power2.easeInOut }, "start");
		if(this.svgMiddle){this.mainTimeline.fromTo(this.svgMiddle, 2, {opacity:0}, { opacity:1 ,ease: Power2.easeInOut}, "start+=2.5");}
		this.mainTimeline.fromTo([this.svgLeft, this.svgRight], 2, {opacity:0}, { opacity:1 ,ease: Power2.easeInOut }, "start+=3.5");
		if(this.svgBottom){this.mainTimeline.fromTo(this.svgBottom, 2, {opacity:0}, { opacity:1 ,ease: Power2.easeInOut }, "start+=4")}
		this.mainTimeline.fromTo($('.ctaContainer'), 1, { width: 0 }, { width:100, ease: Quad.easeOut }, "start+=5.5");
		this.mainTimeline.fromTo([$('.trailerHit'), $('#trailerPlayButton')], 1, {autoAlpha:0}, {autoAlpha:1, ease: Power1.easeInOut }, "start+=6")
		this.mainTimeline.from($('#bg'), 4, { y:75 ,ease: Power2.easeOut, rotationZ:0.1}, "start+=0.3")
		this.mainTimeline.from([$('#professor'), $('#professorDark')], 4, { y:100, rotationZ:0.1 ,ease: Power2.easeOut }, "start+=0.3")
		this.mainTimeline.from($('#money'), 4, { y:175 ,rotationZ:0.1 ,ease: Power2.easeOut }, "start+=0.3")
		this.mainTimeline.from($('#bank'), 4, { y:200, rotationZ:0.1 ,ease: Power2.easeOut, onComplete:function(){this.rolloverAllowed = true;}.bind(this) }, "start+=0.3")
		this.mainTimeline.add("cameraMoveDone", "+=1")
		this.mainTimeline.fromTo($('#bgDark'), 0.05, { opacity:0}, {opacity:0.5, repeat:5, yoyo:true, ease: Power1.easeInOut }, "cameraMoveDone")
		this.mainTimeline.fromTo($('#professorDark'), 0.05, { opacity:0}, {opacity:1, repeat:5, yoyo:true, ease: Power1.easeInOut }, "cameraMoveDone")
		this.mainTimeline.to($('#bgDark'), 0.05, { opacity:0.5 , repeat:3, yoyo:true, ease: Power1.easeInOut }, "cameraMoveDone+=3")
		this.mainTimeline.to($('#professorDark'), 0.05, { opacity:1 , repeat:3, yoyo:true, ease: Power1.easeInOut }, "cameraMoveDone+=3")
		this.mainTimeline.to($('#bgDark'), 0.05, { opacity:0.5 , repeat:1, yoyo:true, ease: Power1.easeInOut }, "cameraMoveDone+=5")
		this.mainTimeline.to($('#professorDark'), 0.05, { opacity:1 , repeat:1, yoyo:true, ease: Power1.easeInOut }, "cameraMoveDone+=5")

		this.trailerTimeline = new TimelineMax({ paused: true, onStart:function(){this.playTrailer();}.bind(this), onReverseComplete:function(){this.trailerScreen = false}.bind(this) });
		this.trailerTimeline.add("start")
		this.trailerTimeline.to([$('.trailerHit'), $('#trailerPlayButton')], 0.5, { autoAlpha:0, ease: Power2.easeOut }, "start")
		this.trailerTimeline.from($('#trailerComponent'), 2, { y:188, rotationZ:0.1, ease: Power1.easeOut }, "start")
	}

	banner.prototype.onOver = function(e) {
		_super.prototype.onOver.call(this);

		// this.movedX = e.offsetX;
		// this.movedY = e.offsetY;
		if(this.rolloverAllowed && !this.trailerScreen){
			// TweenMax.to([$('#bg'), $('#bgDark')], 0.2, {y:-(125 - this.movedY)/50, x:-(150 - this.movedX)/50, rotationZ:0.1});
			// TweenMax.to([$('#professor'), $('#professorDark')], 0.2, {y:-(125 - this.movedY)/46, x:-(150 - this.movedX)/46, rotationZ:0.1});
			// TweenMax.to($('#money'), 0.2, {y:-(125 - this.movedY)/42, x:-(150 - this.movedX)/42, rotationZ:0.1});
			// TweenMax.to($('#bank'), 0.2, {y:-(125 - this.movedY)/40, x:-(150 - this.movedX)/40, rotationZ:0.1});
			if(this.mainTimeline.progress() == 1 && this.blinkRolloverAllowed){
				this.blinkRolloverAllowed = false;
				TweenMax.to($('#bgDark'), 0.05, { opacity:0.5 , repeat:3, yoyo:true, ease: Power1.easeInOut, delay:1, onComplete:function(){this.blinkRolloverAllowed = true;}.bind(this)});
				TweenMax.to($('#professorDark'), 0.05, { opacity:1 , repeat:3, yoyo:true, ease: Power1.easeInOut, delay:1});
			}
		}
	}
	...
	// 	if(this.rolloverAllowed && !this.trailerScreen){
	// 		TweenMax.to([$('#bg'), $('#bgDark')], 0.5, {y:0, rotationZ:0.1});
	// 		TweenMax.to([$('#professor'), $('#professorDark')], 0.5, { y:0, rotationZ:0.1});
	// 		TweenMax.to($('#money'), 0.5, { y:0, rotationZ:0.1});
	// 		TweenMax.to($('#bank'), 0.5, { y:0, rotationZ:0.1});
	// 	}
	// }

	return banner;

})( temple.NetflixBanner );
    ...
    function NetflixVideo(component, config) {
      ...
      function setEvents() {
        ...
        this.closeButton.addEventListener(
          'mouseover',
          function() {
            TweenMax.to(this.closeButton.fill, 0.3, { fill: this.config.closeColors[1], ease: Quad.easeOut });
        ...
        this.closeButton.addEventListener(
          'mouseout',
          function() {
            TweenMax.to(this.closeButton.fill, 0.3, { fill: this.config.closeColors[0], ease: Quad.easeOut });
    ...
    var component = Object.create(HTMLElement.prototype, {
      ...
      preview: {
        value: function() {
          ...
          this.setAttribute('source', 'http://ae.nflximg.net/monet/video/fpo/1x1.mp4');
        }
      }
    });
...
(function() {
  ...
  if (document.registerElement) {
    var component = Object.create(HTMLElement.prototype, {
          ...
          if (MonetComponent) {
            ...
            MonetComponent.getMonetData().then(function(data) {
              ...
              try {
                ...
                if (/\.(jpe?g|png|gif|svg)$/i.test(bindSrc)) {
                  imgPath = bindSrc;
      ...
      preview: {
        value: function() {
          var imgPath = 'https://ae.nflximg.net/monet/img/c20/netflix_placement.svg';
